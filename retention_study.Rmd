...
*** STUDENT_LIST.FORMER_CONCURRENT, ***
STUDENT_TERMS.PIDM, 
...
inst <- ret_data %>% 
  ...
  group_by(PIDM) %>%
  summarise(inst = mean(instrc_type_dum)) %>%
  ungroup()
...
engl <- ret_data %>% 
  ...
  filter(TERM_CODE != 201720 | TERM_CODE != 201730 | TERM_CODE != 201640) %>%
  summarise(engl = max(COURSE_NUMB)) %>% 
  ungroup() 
...
math <- ret_data %>%
  ...
  filter(TERM_CODE != 201720 | TERM_CODE != 201730 | TERM_CODE != 201640) %>%
  summarise(math = max(COURSE_NUMB)) %>%
  ungroup()
...
fed <- ret_data %>%
  group_by(PIDM) %>%
  filter(TERM_CODE != 201720 | TERM_CODE != 201730 | TERM_CODE != 201640) %>%
  summarise(fed_career = sum(FED_LOAN)) %>%
  ungroup
...
pri <- ret_data %>%
  group_by(PIDM) %>%
  filter(TERM_CODE != 201720 | TERM_CODE != 201730 | TERM_CODE != 201640) %>%
  summarise(pri_career = sum(PRIVATE_LOAN)) %>%
  ungroup
...
fails <- ret_data %>%
  ...
  summarise(fails_term = max(CREDITS_FAILED))

fails_career <- fails %>%
  group_by(PIDM) %>%
  filter(TERM_CODE != 201720 | TERM_CODE != 201730 | TERM_CODE != 201640) %>%
  summarise(fails_career = sum(fails_term))
...
withs <- ret_data %>%
  group_by(PIDM, TERM_CODE) %>%
  summarise(with_term = max(CREDITS_WITHDRAWN))

withs_career <- withs %>%
  group_by(PIDM) %>%
  filter(TERM_CODE != 201720 | TERM_CODE != 201730 | TERM_CODE != 201640) %>%
  summarise(withs_career = sum(with_term))
...
mean_crd_att <- ret_data %>%
  group_by(PIDM) %>%
  filter(TERM_CODE != 201720 | TERM_CODE != 201730 | TERM_CODE != 201640) %>%
  summarise(mean_crd_att = mean(CREDITS_ATT))
...
student_term <- ret_data %>% group_by(PIDM, TERM_CODE) %>% 
  distinct(FED_LOAN, CREDITS_ATT, FAFSA, 
           ...
           LAST_TERM, term_age, engl, math, inst,
           ret1720, ret1740, fails_career, withs_career, mean_crd_att)
...
colnames(full_data)[40:41] <- c("cte_courses", "rem_courses")
full_data <- full_data %>% left_join(cte_career, by = "PIDM")
full_data <- full_data %>% left_join(rem_career, by = "PIDM")
colnames(full_data)[42:43] <- c("cte_career", "rem_career")
...
rm(awards, cte_courses, rem_courses, cte_career, rem_career, 
   ...
   fed, inst, pri, withs, withs_career, fails, fails_career)
```
...
# the 201640 variables to match on (not impacted by treatment): S_entry_action is problematic (9 different types)
elig_2016_set_cs <- full_data_awards %>% 
  ...
  select(PIDM, TERM_CODE, term_age, first_gen, eth, male, 
         ...
         avg_cls_sz, zip, mean_crd_att, first_sem) 

elig_2016_set_ff <- full_data_awards %>% 
  filter(TERM_CODE < 201700) %>% 
  filter(is.na(award_flag) == T) %>%
  filter(TERM_CODE == 201640) %>%
  filter(first_sem == 1) %>%
...
# 5975 total students in both sets. 3933 non-first sem 2042 for first sem
...
Generate propensity scores for all exact match on minimum cohort year. match on zip.
- how to flag students who have tested out of Math courses? 
- hos to flag students who haven't taken either math or engl?
...
- former concurrent from student_list when you get back. 

# consideration

- run first semesters seperately 
